...
namespace KPMG.Pulse.Back.Accounting.Mandate.Adapters.Tests
{
    ...
    public class FormioAdapterTest
    {
        ...
        public async Task GetAllCollectionAsync()
        {
            ...
            var formIoClient = new Mock<IFormioClient>(MockBehavior.Strict);
            formIoClient.Setup(item => item.GetSubmissionsAsync(0, 1, It.IsAny<FormioAuthToken>()))
                .ReturnsAsync(sub)
                .Verifiable();

            var adapter = new FormioAdapter(formIoClient.Object);

            var res = await adapter.GetAllCollectionAsync(0, 1);
            Bban bban1 = new Bban("13507", "00014", "31464482121", "77", "8909441", null);
            var address = new Address("12 RUE DES 2 NATIONS", string.Empty, "59250", "HALLUIN", "France");
            var signatory = new Signatory("m", "OLIVIER", "BRUNELAT", "toto@gmail.com");
            var company = new Company(default, "SPORT FIT SAS", "83455379400019", "1000326214", "19820673", signatory, address);
        ...
        [Fact]
        public async Task GetAllCollectionAsync_When_GetSubmissionMandateAsync_Return_NoResult()
        {
            Bban bban = new Bban("13507", "00014", "31464482121", "77", "8909440", null);
            var sub = new FormioSubmissionCollection()
            {
            ...
            res.Should().BeEquivalentTo(new List<Collection>());

            formIoClient.VerifyAll();
        }

        [Fact]
        public async Task GetAllCollectionAsync_When_GetSubmissionMandateAsync_Throw_Exception()
        {
            var formIoClient = new Mock<IFormioClient>(MockBehavior.Strict);
            formIoClient.Setup(item => item.GetSubmissionsAsync(0, 1, It.IsAny<FormioAuthToken>()))
            ...
            Func<Task> action = async () => await adapter.GetAllCollectionAsync(0, 1);
