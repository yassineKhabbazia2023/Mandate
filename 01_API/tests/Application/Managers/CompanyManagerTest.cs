...
namespace KPMG.Pulse.Back.Accounting.Mandate.Application.Tests.Managers
{
    public class CompanyManagerTest
    {
        ...
        public async Task GetCompanyByErpIdAsync_ShouldReturnCompany_WhenErpIdIsValid()
        {
            ...
            databaseServiceMock.Setup(service => service.GetCompanyByErpIdAsync(erpId, userEmail))
            ...
            // Act
            var result = await companyManager.GetCompanyByErpIdAsync(erpId, userEmail);

            // Assert
            result.Should().BeEquivalentTo(expectedCompany, options => options.ComparingByMembers<Company>());
            databaseServiceMock.Verify(service => service.GetCompanyByErpIdAsync(erpId, userEmail), Times.Once);
        }
    }
}
